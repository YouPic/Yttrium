...
class ProfilePoint(val server: String, val duration: Long, val events: List<Event>)
...
class MetricStore {
    ...
    @Synchronized fun onStat(packet: StatPacket) {
        ...
        val point = if(existing === null) {
            val p = Metric(packet.path, packet.time)
            timeMap.put(key, p)
            inFlightTimes.add(p)
            p
        } else existing
    ...
    @Synchronized fun onProfile(packet: ProfilePacket) {
        ...
        val profile = if(existing === null) {
            val p = Profile(packet.path, packet.time)
        ...
        var insertIndex = 0
        profile.profileBuilder.find {insertIndex++; it.duration > duration}
        profile.profileBuilder.add(insertIndex - 1, ProfilePoint(packet.server, duration, packet.events))
